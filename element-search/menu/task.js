...
let menuLinkArr = Array.from(menuLink); // не знаю нужно ли это преобразование в массив

for (let i = 0; i < menuLinkArr.length; i += 1) { // 1. цикл для перебора псевдо массива
    let menuItem = menuLinkArr[i].closest('.menu__item'); // находим родителя .menu__item
    ...
    let menu = menuLinkArr[i].closest('.menu'); // находим родителя .menu
    ...
    if (menuSub) { // 2. условие, если есть дочерний элемент .menu_sub
        menuLinkArr[i].onclick = function() { // назначаем функцию на клик .menu__link
